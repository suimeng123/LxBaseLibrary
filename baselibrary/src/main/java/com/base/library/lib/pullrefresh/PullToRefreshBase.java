 ...
 *******************************************************************************/
package com.base.library.lib.pullrefresh;
...
import com.base.library.lib.internal.FlipLoadingLayout;
import com.base.library.lib.internal.LoadingLayout;
import com.base.library.lib.internal.RotateLoadingLayout;
import com.base.library.lib.internal.Utils;
import com.base.library.lib.internal.ViewCompat;
				...
				if (mIsBeingDragged) {
					...
					if (isRefreshing()) {
						if(mRefreshableView instanceof AbsListView) {
							ListAdapter adapter = ((AbsListView)mRefreshableView).getAdapter();
...
//				if(lastState != State.REFRESHING && lastState != State.MANUAL_REFRESHING) {
					onRefreshing(params[0]);
//				} else {
//					PullToRefreshAdapterViewBase.this.onRefreshing(doScroll);
//					PullToRefreshBase.this.onRefreshing(params[0]);
//				}
				break;
			case OVERSCROLLING:
				// NO-OP
	...
	protected final void setHeaderScroll(int value) {
		...
		if (USE_HW_LAYERS && Build.VERSION.SDK_INT >= Build.VERSION_CODES.HONEYCOMB) {
			/**
	...
	private void init(Context context, AttributeSet attrs) {
		...
		// Styleables from XML
		TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.EcmPullToRefresh);

		if (a.hasValue(R.styleable.EcmPullToRefresh_ptrMode)) {
			mMode = Mode.mapIntToValue(a.getInteger(R.styleable.EcmPullToRefresh_ptrMode, 0));
		}

		if (a.hasValue(R.styleable.EcmPullToRefresh_ptrAnimationStyle)) {
			mLoadingAnimationStyle = AnimationStyle.mapIntToValue(a.getInteger(
					R.styleable.EcmPullToRefresh_ptrAnimationStyle, 0));
		}
		 ...
		 * Styleables from XML
		 */
		if (a.hasValue(R.styleable.EcmPullToRefresh_ptrRefreshableViewBackground)) {
			Drawable background = a.getDrawable(R.styleable.EcmPullToRefresh_ptrRefreshableViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		} else if (a.hasValue(R.styleable.EcmPullToRefresh_ptrAdapterViewBackground)) {
			Utils.warnDeprecation("ptrAdapterViewBackground", "ptrRefreshableViewBackground");
			Drawable background = a.getDrawable(R.styleable.EcmPullToRefresh_ptrAdapterViewBackground);
			if (null != background) {
				mRefreshableView.setBackgroundDrawable(background);
			}
		}

		if (a.hasValue(R.styleable.EcmPullToRefresh_ptrOverScroll)) {
			mOverScrollEnabled = a.getBoolean(R.styleable.EcmPullToRefresh_ptrOverScroll, true);
		}

		if (a.hasValue(R.styleable.EcmPullToRefresh_ptrScrollingWhileRefreshingEnabled)) {
			mScrollingWhileRefreshingEnabled = a.getBoolean(
					R.styleable.EcmPullToRefresh_ptrScrollingWhileRefreshingEnabled, false);
		}
	...
	private void pullEvent() {
		...
		setHeaderScroll(newScrollValue+starty);
